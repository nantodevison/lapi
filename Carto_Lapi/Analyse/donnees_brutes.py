...
def ouvrir_fichier_lapi(date_debut, date_fin) : 
    ...
    with ct.ConnexionBdd('gti_lapi') as c : 
        requete=f"select case when camera_id=13 or camera_id=14 then 13 when camera_id=15 or camera_id=16 then 15 else camera_id end::integer as camera_id , created, immatriculation as immat, fiability, l, state from data.te_passage where created between '{date_debut}' and '{date_fin}'"
...
def ouvrir_fichier_lapi_final() : 
    ...
    with ct.ConnexionBdd('gti_lapi_final') as c : 
        ...
        df_passage=pd.read_sql_query(requete, c.sqlAlchemyConn)
        requete_plaque=f"select plaque_ouverte, chiffree from data.te_plaque_longue"
